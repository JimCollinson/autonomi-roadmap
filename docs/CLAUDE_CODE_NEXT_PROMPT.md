# Claude Code next prompt — Autonomi roadmap shim

Use this prompt with Claude Code from the repo root:
...
- This repo is currently local only. It should be published under Jim's GitHub account as `jimcollinson/autonomi-roadmap` unless Jim says otherwise.
...
2. Add ADRKit governance and an ADR for the roadmap-content-shim decision.
...
Consider adding a GitHub Pages or Actions artifact preview later, but do not overbuild unless quick.
...
If Framer MCP is available, use it to create/update a test page on autonomy.com with `framer/RoadmapBody.tsx` and those URLs.
